...
#define MIC_VOL                 0x55	
#define SPI_MODE_MASK 0x0C  // CPOL = bit 3, CPHA 设置= bit 2 on SPCR
...
public:
  ...
  void MO();
  void Input();
  inline static byte transfer(byte _data);  //合适可以读SPDR里面的数据
...
byte VoiceClass::transfer(byte _data) {
  SPDR = _data;   //赋值
  while (!(SPSR & _BV(SPIF)))//　　_Bv(bit)：比特价值，将特定位数作为字节值进行访问,相当与(1<<bit):即将1左移bit位，判断SPSR向　系ｉａｂｇ向地址想想向向０向、、０＝＝、、设置为高电平为、、热线儿童、ｒｅｓｅｔＳＰＥ、、　　、、色图设置的SPIF这一位是不是１，如果不是１，则继续循环，是１，则退出循环
    　　;
  return SPDR;　//返回_data数据
}

void VoiceClass::attachInterrupt() {
  SPCR |= _BV(SPIE);   //设置SPCR的SPIE位为１，希望在串行传输完成时执行SPI中断
}

void VoiceClass::detachInterrupt() {
  SPCR &= ~_BV(SPIE);　//检测SPCR的SPIE位是否为１，以判断串行传输完成时是否执行SPI中断
}
